...
export const getListPage = async (filePath) => {

  {
    try {
      ...
      const pageDataParsed = document.exists ? document.data() : NULL;

      const docRefErr = doc(db, "content/403");
      const documentErr = await getDoc(docRefErr);
      const notFoundDataParsed = documentErr.exists ? documentErr.data() : null;

      let frontmatter, content;
      if (pageDataParsed) {
        content = pageDataParsed;
        frontmatter = pageDataParsed;
      } else {
        content = notFoundDataParsed;
        frontmatter = notFoundDataParsed;
      }
      const mdxContent = await parseMDX(content);
...
export const getSinglePage = async (folder) => {
  ...
  const publishedPages = posts.filter(
    (page) =>
      !page.frontmatter.draft && page.frontmatter.layout !== "403" && page
  );
  const filterByDate = publishedPages.filter(
    (page) => new Date(page.frontmatter.date || new Date()) <= new Date()
  );
...
export const getRegularPage = async (slug) => {
  ...
  // const notFoundPage = fs.readFileSync("content/404.md", "utf-8");
  const notFoundDataParsed = await getListPage("content/403")
  let frontmatter, content;
  if (pageData[0]) {
